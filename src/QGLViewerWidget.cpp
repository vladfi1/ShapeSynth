...
QGLViewerWidget::init(void)
{
    ...
    // popup menu for draw modes
    
    popup_menu_ = new QMenu(this);
    draw_modes_group_ = new QActionGroup(this);
    
    connect( draw_modes_group_, SIGNAL(triggered(QAction*)), this, SLOT(slotDrawMode(QAction*)));
    ...
    // popup menu for show modes
    
    popupShowModeMenu_ = new QMenu(this);
    showModesGroup_ = new QActionGroup(this);
    
    QObject::connect( showModesGroup_, SIGNAL(triggered(QAction*)), this, SLOT(slotChangeShowMode(QAction*)));
    ...
    QAction *b;
    b = addShowMode("Boxes");
    b = addShowMode("Points");
    b = addShowMode("Mesh");
    b = addShowMode("Boxes and Points");
    b = addShowMode("Mesh and Points");
    b = addShowMode("Boxes and Mesh");
    b = addShowMode("Boxes Points and Mesh");
    b->setChecked(true);
    
    slotChangeShowMode(b);
...
int QGLViewerWidget::pick(int x, int y)
{
    // enable GL context
    makeCurrent();
    ...
    GLuint buff[64] = {0};
 	...
 	/*
     This chooses the buffer where store the values for the selection data
     */
    ...
    /*
     We pushed matrix in PROJECTION mode
     */
    glMatrixMode(GL_PROJECTION);
 	glPopMatrix();
     ...
     */
 	//list_hits(hits, buff);
    unsigned int topHitID =  processHits(hits, buff);
    ...
    //unsigned int topHitID = buff[0 * 4 + 3];
...
//----------------------------------------------------------------------------
void QGLViewerWidget::pick_scene()
{
    return;
}

void QGLViewerWidget::list_hits(GLint hits, GLuint *names)
{
 	int i;
...
unsigned int QGLViewerWidget::processHits(GLint hits, GLuint buffer[])
{
    unsigned int i, j;
    GLuint names, *ptr;
    ...
    unsigned int topSelection = 0;
    
    GLuint minZ = std::numeric_limits<GLuint>::max();
    ...
    for (i = 0; i < hits; i++) {  /* for each hit  */
        ...
        //printf("z1 is %g;", (float) z1/std::numeric_limits<GLuint>::max());
        ptr++;
        
        GLuint z2 = *ptr;
        
        //printf("z2 is %g\n", (float) z2/std::numeric_limits<GLuint>::max());
        ptr++;
...
//----------------------------------------------------------------------------
void QGLViewerWidget::processHover(int posx, int posy)
{
    int topID = pick(posx, posy); //size().height() - _event->pos().y());
    
    emit hoveredShape(topID);
...
QGLViewerWidget::mousePressEvent( QMouseEvent* _event )
{
    ...
    if (_event->button() == RightButton && _event->buttons()== RightButton && _event->modifiers() == Qt::ShiftModifier)
    {
        //popup_menu_->exec(QCursor::pos());
        int topID = pick(_event->pos().x(), size().height() - _event->pos().y());
        if (topID >= 0)
        {
            emit pickedShapeShowNext(topID);
        }
        
    }
    else if (_event->button() == LeftButton && _event->buttons()== LeftButton && _event->modifiers() == Qt::ShiftModifier)
    {
        //popup_menu_->exec(QCursor::pos());
        int topID = pick(_event->pos().x(), size().height() - _event->pos().y());
        if (topID >= 0)
        {
            emit pickedShapeShowPrevious(topID);
        }
...
QGLViewerWidget::mouseMoveEvent( QMouseEvent* _event )
{
    ...
    {
        processHover(_event->pos().x(), size().height()- _event->pos().y());
...
void QGLViewerWidget::keyPressEvent( QKeyEvent* _event)
{
    switch( _event->key() )
    {
        ...
        case Key_H:
            qDebug() << "Keys:\n";
            qDebug() << "  Print\tMake snapshot\n";
            qDebug() << "  C\tenable/disable back face culling\n";
            qDebug() << "  F\tenable/disable fog\n";
            qDebug() << "  I\tDisplay information\n";
            qDebug() << "  N\tenable/disable display of vertex normals\n";
            qDebug() << "  Shift N\tenable/disable display of face normals\n";
            qDebug() << "  Shift P\tperformance check\n";
            break;
        ...
        case Key_C:
            if ( glIsEnabled( GL_CULL_FACE ) )
            {
                ...
                qDebug() << "Back face culling: disabled\n";
            }
            else
            {
                glEnable( GL_CULL_FACE );
                qDebug() << "Back face culling: enabled\n";
            }
            updateGL();
            break;
        ...
        case Key_I:
            qDebug() << "Scene radius: " << radius_ ;
            qDebug() << "Scene center: " << center_[0] << " " << center_[1] << " " << center_[2];
            break;
        ...
        case Key_P:
            if (_event->modifiers() & ShiftModifier)
            {
                ...
                qDebug() << "fps: " << /*std::setiosflags (std::ios_base::fixed) << */ fps ;
            }
            break;
...
//----------------------------------------------------------------------------
QAction* QGLViewerWidget::addShowMode(const std::string& _s)
{
    ++nShowModes_;
    showModeNames_.push_back(_s);
    
    QActionGroup *grp = showModesGroup_;
    
    QAction* act = new QAction(tr(_s.c_str()), this);
    act->setCheckable(true);
    act->setData(nShowModes_);
    
    grp->addAction(act);
    popupShowModeMenu_->addAction(act);
...
// popup menu clicked
void QGLViewerWidget::slotChangeShowMode(QAction *_mode)
{
    // save draw mode
    showMode_ = _mode->data().toInt();
    updateGL();
    
    // check selected show mode
    //popupShowModeMenu_->setItemChecked(showMode_, true);
}
//----------------------------------------------------------------------------
