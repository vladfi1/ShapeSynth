 ...
 *                                                                           *
 *   $Revision: 809 $                                                         *
 *   $Date: 2013-02-25 14:30:56 +0100 (Mo, 25. Feb 2013) $                   *
 *                                                                           *
\*===========================================================================*/
...
{
  Options opt;
  return read_mesh(_mesh, _filename, opt, true, std::map<unsigned int,unsigned int>());
}
...
read_mesh(Mesh&         _mesh,
	  ...
	  Options&            _opt,
	  bool                _clear = true,
      std::map<unsigned int,unsigned int>* _indexMap = 0 )
{
  if (_clear) _mesh.clear();
  ImporterT<Mesh> importer(_mesh);
  bool success = IOManager().read(_filename, importer, _opt);
  (*_indexMap) = importer.oldIndex2newIndex();
  return success;
}
      ...
      std::map<unsigned int,unsigned int>* _indexMap = 0 )
{
  if (_clear) _mesh.clear();
  ImporterT<Mesh> importer(_mesh);
  bool success = IOManager().read(_is,_ext, importer, _opt);
  (*_indexMap) = importer.oldIndex2newIndex();
  return success;
}
