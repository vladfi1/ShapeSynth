...
public:
  ...
  virtual FaceHandle add_face(const VHandles& _indices, unsigned int _faceIndex = 0)
  {
    FaceHandle fh;
      ...
      for (it=_indices.begin(); it!=end; ++it)
        for (it2=it+1; it2!=end; ++it2)
          if (*it == *it2)
          {
            ...
            failedIndices_.push_back(_faceIndex);
            return fh;
          }
    ...
    //if we reached here then the face was added successfully, so it should map to its original index
    oldIndex2newIndex_[noFaces_] = _faceIndex;
    noFaces_ ++;
  ...
  void prepare() { failed_faces_.clear(); failedIndices_.clear(); oldIndex2newIndex_.clear(); }
  ...
  void finish()
  {
    if (!failed_faces_.empty())
    {
      ...
      for (unsigned int i=0; i<failed_faces_.size(); ++i)
      {
        ...
        oldIndex2newIndex_[  noFaces_]  = failedIndices_[i] ;
      ...
      failed_faces_.clear();
      failedIndices_.clear();
    }
  }

  std::map<unsigned int, unsigned int>& oldIndex2newIndex()
  {
        return oldIndex2newIndex_;
  }
...
private:
  ...
  std::vector<unsigned int> failedIndices_;
  unsigned int noFaces_ = 0;
  // stores normals for halfedges of the next face
  std::map<VertexHandle,Normal> halfedgeNormals_;
    
  std::map<unsigned int, unsigned int> oldIndex2newIndex_;
};
